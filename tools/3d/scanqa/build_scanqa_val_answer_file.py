...
for entry in data:
    ...
    formatted_entry = {
        ...
        "text": entry.get("answers", []),
        "video": entry.get("scene_id", "")
    }
    formatted_questions.append(formatted_entry)
    question_id += 1

with open("/depot/schaterj/data/3d/work_dir/zhuoming_temp/run_llama/data/video_instruction_tuning/scannet/llava3d_scanqa_val_answer.json", "w") as outfile:
    json.dump(formatted_questions, outfile, indent=4)


# python tools/prepare_3d/scanqa/build_scanqa_val_question_file.py