...
class BaseDataset(Dataset):
    """Base class for video dataset with caption"""
    ...
    def __init__(
        ...
        vis_root: root directory of video files
        """
        ...
        self.anns = self.load_annotation_and_filter()

        # by default, we use imagebind to extract video features
        self.feat_root = vis_root.replace("Charades_v1_480", "Charades_v1_audio_imagebind_feat")

        self.transform = transforms.Compose(
            [
                transforms.Resize((max_img_size, max_img_size), interpolation=InterpolationMode.BICUBIC),
                transforms.ToTensor(),
                transforms.Normalize((0.48145466, 0.4578275, 0.40821073), (0.26862954, 0.26130258, 0.27577711)),
            ]
        )

    def load_annotation_and_filter(self):
        anns = []
        for ann_path in self.ann_paths:
            if "avsd" in ann_path:
                # Load the video mapping file
                mapping_file = os.path.join(os.path.dirname(ann_path), "avsd_all_videos_mapping.json")
            ...
            if ann_path.endswith(".json"):
                ...
                raise ValueError("Unknown annotation file type: {}".format(ann_path))

        # Filter annotations based on whether the video files exist
        filtered_anns = []
        for ann in anns:
            video = ann["video"]
            # Handle AVSD video mapping
            if "avsd" in ann_path and video in video_mapping:
                mapped_video = video_mapping[video]
                video_path = os.path.join(self.vis_root, mapped_video)
            else:
                video_path = os.path.join(self.vis_root, video)

            if os.path.exists(video_path):
                filtered_anns.append(ann)

        LOGGER.info("Find {} samples in {}".format(len(filtered_anns), ann_path))
...
def load_annotation_and_filter(anno_path, fast_path_mapping_path, data_root, 
    ...
    list_data_dict = load_data(anno_path) # It will be a list and each ele in a list is a dictionary
    # load mappings
    ...
    # filter for id that video not exist
    filtered_anno_list = []
    remaining_video = []
    video_not_exist = []
    ...
    for ele in list_data_dict:    
        # decode video id
        vid = ele['video']
        # handle spcial case in LLaVA-video 178K
        if '/' in vid:
            vid = vid.split('/')[-1]        
        # handle for case that for activitynet video of sharegpt-video
        if 'Scene' in vid: 
            vid = vid[:13]
        # handle for special case for valley and video-chatgpt dataset       
        ...
        # <--- 修改开始 ---
        # Handle fast feature path (for .pt files)
        if vid_with_ext in feat_path_mapping:
            raw_feat_path = feat_path_mapping[vid_with_ext]
            
            # Use os.path.normpath to normalize the path (e.g., removes redundant './')
            norm_feat_path = os.path.normpath(raw_feat_path)
            
            # Check if the path is absolute
            if os.path.isabs(norm_feat_path):
                # If it's an absolute path, use it directly
                ele['feat_path'] = norm_feat_path
            else:
                # If it's a relative path, join it with the data_root
                ele['feat_path'] = os.path.join(data_root, norm_feat_path)
        # <--- 修改结束 ---

        # Handle slow video path (for .mp4 files)
        if video_path_mapping is not None: # handle for the special case that also consider raw video
            video_file_path = os.path.join(slow_path_data_root, video_path_mapping[vid_with_ext])
        ...
        if second_side_channels_root is not None: # this should be a feature
            second_side_file_path = os.path.join(second_side_channels_root, feat_path_mapping[vid_with_ext])
            # handle for special case for the root
            if 'ego' in second_side_file_path: # this should be a feature
                second_side_file_path = '/'.join(second_side_file_path.split('/')[:-1])
                
            if not os.path.exists(second_side_file_path): # filter for not exist video
                video_not_exist.append(vid)
                continue
    ...
    # 添加错误检测机制：如果过滤后没有剩余数据，则抛出异常
    if len(filtered_anno_list) == 0:
        raise ValueError(f"No valid samples found in dataset. "
                         f"Total annotations: {len(list_data_dict)}, "
                         f"Videos not existing: {len(set(video_not_exist))}. "
                         f"Please check your data paths and mappings.")
