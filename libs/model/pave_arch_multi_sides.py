# This script holds the implementation of the PAVE model with multiple side-channel information.
...
class PAVEMultiSidesMetaModel:

    def __init__(self, config):
        super(PAVEMultiSidesMetaModel, self).__init__(config)
    ...
    def initialize_vision_modules(self, model_args, fsdp=None):
        # import ipdb
        # ipdb.set_trace() # check the module init
        #### The Fast-path Init ###
        ...
        ############## build the temporal aggregator2 
        self.config.temporal_aggregator2_type = getattr(model_args, 'temporal_aggregator2_type', 'ssm')
        if getattr(self, 'temporal_aggregator2', None) is None:
            self.temporal_aggregator2 = build_temporal_aggregator(model_args, second_module=True)
        else:
            # In case it is frozen by LoRA
            for p in self.temporal_aggregator2.parameters():
                p.requires_grad = True


        # load the existing temporal aggregator checkpoint (for step 2 training): this should be skipped
        # ipdb.set_trace() # check the module should be skip
...
class PAVEMultiSidesMetaForCausalLM(ABC):
    ...
    def encode_second(self, 
                      second_feats, 
                      q_text_embeds=None, 
                      second_feat_fps=None, 
                      second_feat_frame_nums=None,
                      q_text_nums=None,
                      chunk_num=None,
                      slow_feats=None):
        # video_feats         : Tensor [B, C, T, H, W] 
        ...
        second_feats = second_feats.permute([0,2,3,4,1]) # [B, C, T, H, W] -> [B, T, H, W, C]
        
        # pass the feature to the temporal aggregator
        if hasattr(self.get_model().temporal_aggregator2, 'use_slow_as_query') and self.get_model().temporal_aggregator2.use_slow_as_query:
            second_feats = self.get_model().temporal_aggregator2(second_feats, second_feat_frame_nums, 
                                                                q_text_embeds=None, 
                                                                q_text_nums=None,
                                                                chunk_num=chunk_num,
                                                                slow_feats=slow_feats)
        ...
        return second_feats, new_frame_num
    ...
    def prepare_inputs_labels_for_multimodal(
                ...
                second_feats=None, # added for multi-sides
                second_feat_fps=None,
                second_feat_frame_nums=None,
            ):
        ...
        if video_feats is None:
            video_features, _ = None, None
        else:
            video_features, _ = self.encode_videos(video_feats, 
        ...
        ############################################### get the second feature (the second side channel) ##################################################################

        if second_feats is None:
            second_features, _ = None, None
        else:
            second_features, _ = self.encode_second(second_feats, 
                                                            q_text_embeds=question_embeds,
                                                            second_feat_fps=second_feat_fps, 
                                                            second_feat_frame_nums=second_feat_frame_nums,
                                                            q_text_nums=question_lens,
                                                            chunk_num=chunk_num,
                                                            slow_feats=image_features if not self.use_slow_feat_before_mlp else image_features_before_mlp) # torch.Size([2, 6272, 896])
        ...
        feat_combine_method = getattr(self.config, 'feat_combine_method', 'add')
        if video_features is not None and second_features is not None and feat_combine_method == 'add':
            assert image_features[0].shape[1]*image_features[0].shape[0] == video_features.shape[1]
            assert image_features[0].shape[1]*image_features[0].shape[0] == second_features.shape[1]
            
            updated_image_feat = []
            # add up the feature
            for curr_second_feat, curr_video_feat, curr_image_feat in zip(second_features, video_features, image_features):
                curr_second_feat = rearrange(curr_second_feat, "(k s) d -> k s d", k=chunk_num)  # flatten the spatial
                curr_video_feat = rearrange(curr_video_feat, "(k s) d -> k s d", k=chunk_num)  # flatten the spatial
                updated_image_feat.append(curr_second_feat + curr_video_feat + curr_image_feat)
